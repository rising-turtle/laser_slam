...
public:
	...
	Bayesian_filter_matrix::Float normal(const Float mean, const Float sigma)
	{
		...
		boost::variate_generator<URng&, boost::normal_distribution<Float> > gen(rng, dist);
		// return dist(rng);
		return gen();
	}
	...
	{
		boost::variate_generator<URng&, boost::normal_distribution<Float> > gen(rng, dist_normal_01);
		std::generate (v.begin(), v.end(), gen);
	}
	void uniform_01(Bayesian_filter_matrix::DenseVec& v)
	{
		boost::variate_generator<URng&, boost::uniform_01<Float> > gen(rng, dist_uniform_01);
		std::generate (v.begin(), v.end(), gen);
	}
	...
	{
		boost::normal_distribution<Float> dist(mean, sigma);
		boost::variate_generator_01<URng&, boost::normal_distribution<Float> > gen(rng, dist);
		for (std::size_t i = 0, iend=v.size(); i < iend; ++i)
			v[i] = gen();
	}
	void normal(Bayesian_filter_matrix::Vec& v)
	{
		boost::variate_generator_01<URng&, boost::normal_distribution<Float> > gen(rng, dist_normal_01);
		for (std::size_t i = 0, iend=v.size(); i < iend; ++i)
			v[i] = gen();
	}
