...
void Unscented_scheme::updateFusion ()
/* Update state
 *  Pre : x,X
 *  Post: x,X
 */
{
	...
	rclimit.check_PD(rcond, "Y not PD in updateFusion");
	noalias(x) = prod(X, y);
 ...
 */
{
	...
	//init fusion info contribution constants
 ...
 */
{
	...
	//printf("UUUU BEFORE: %f, %f, %f",X(0,0), X(1,1), X(2,2));
						// Filter update
	noalias(x) += prod(W,s);
	RowMatrix WStemp(W.size1(), S.size2());
	noalias(X) -= prod_SPD(W,S, WStemp);

	//printf("UUUU AFTER: %f, %f, %f",X(0,0), X(1,1), X(2,2));

	return rcond;
}

Bayes_base::Float Unscented_scheme::observeFusion (Uncorrelated_additive_observe_model& h, const FM::Vec& z)
/* Observation fusion
 *  Pre : x,X
 *  Post: x,X is PSD
 *
...
{
	Adapted_Correlated_additive_observe_model hh(h);
	return observeFusion (hh, z);
}


Bayes_base::Float Unscented_scheme::observeFusion (Correlated_additive_observe_model& h, const FM::Vec& z)
/* Observation fusion
 *  Pre : x,X
 *  Post: x,X is PSD
 */
{
	...
	//calc info vector and matrix

	Float rcond = UdUinversePD (Y, X);
	rclimit.check_PD(rcond, "X not PD in observeFusion");
	noalias(y) = prod(Y,x);
	//cout<<"y: "<<y[0]<<" "<<y[1]<<" "<<y[2]<<endl;

	SymMatrix RI(z_size, z_size);
	rcond = UdUinversePD (RI, h.Z);
	rclimit.check_PD(rcond, "h.Z not PD in observeFusion");
	...
	*/
	...
	Matrix YP(x_size, z_size);
	noalias(YP)= prod(Y, Xxz);
	RowMatrix WStemp(YP.size1(), RI.size2());
	noalias(Y_ick) += prod_SPD(YP, RI, WStemp);

	noalias(s) += prod(trans(Xxz), y);
	noalias(y_ick) += prod(prod(YP, RI), s);


	/*

	printf("start to record \n");
	ofstream outfile("obsFusion.txt");
	...
	//ofs<<"XY:: "<<x<<" "<<X<<" "<<y<<" "<<Y<<" "<<y_ick<<" "<<Y_ick<<endl;
	//ofs<<"TT:: "<<Xxz<<" "<<YP<<" "<<RI<<" "<<s<<std::endl;
