...
CPoints::CPoints(QWidget* parent):
QGLWidget(parent),
m_IsReady(false),
m_bDrawLine(false)
{
...
void CPoints::receLine(float sx, float sy, float ex, float ey ){
	/*lsx.push_back(sx);
	lsy.push_back(sy);
	lex.push_back(ex);
	ley.push_back(ey);*/
...
void CPoints::recePMAP(float* fx, float* fy, int n, double rx, double ry, double th)
{
	cout<<"recePMAP: "<<n<<" "<<rx<<" "<<ry<<endl;
	vector<float> x(n);
	vector<float> y(n);
	copy(fx,fx+n,x.begin());
	copy(fy,fy+n,y.begin());
	computeTransformation(rx,ry,th,x,y);
	p_px.insert(p_px.end(),x.begin(),x.end());
	p_py.insert(p_py.end(),y.begin(),y.end());
	m_IsReady = true;
}
...
void CPoints::drawMap()
{
	...
	glBegin(GL_POINTS);
	qglColor(Qt::green);
	for(int i=0;i<p_px.size();i++)
		glVertex2f(p_px[i],p_py[i]);
	glEnd();
	...
	if(m_bDrawLine){
		...
		qglColor(Qt::green);
		glBegin(GL_LINES);
		glVertex2f(m_slsx,m_slsy);
		glVertex2f(m_slex,m_sley);
		glEnd();
	}
}
