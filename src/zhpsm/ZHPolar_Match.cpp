...
{
	if(pm_fi!=0) { delete []pm_fi; pm_fi = 0;}
	if(pm_si!=0) { delete []pm_si; pm_si = 0;}
	if(pm_co!=0) { delete []pm_co; pm_co = 0;}
	clearRecord();
}
...
// interface for matching two scans, return the registration error
pair<float,float> CPolarMatch::FMatch(PMScan* lsr, PMScan* lsa, bool bFirst)
{
	...
	try{
		// pm_psm(lsr,lsa);
		pm_icp(lsr,lsa);
		return pm_error_index2(lsr,lsa);
	}catch (int err){
		psm_matched = false;
	}
	if(!psm_matched){
		try{
			// pm_psm(lsr,lsa);
			 pm_icp(lsr,lsa);
			 // if psm failed, this should result in big covariance
			 // return pm_error_index2(lsr,lsa);
			 return make_pair(-1.0, 1.0);
		}catch(int err){
...
bool CPolarMatch::readRawSeed(string filename)
{
	if(!m_bReady){
	...
	cout<<"m_SickScans.size(): "<<m_SickScans.size()<<endl;
	while(inf.getline(line,4096) && cnt<20 ){
		cnt++;
		timestamp = atof(strtok(line,delim.c_str()));
		num_points = atoi(strtok(NULL,delim.c_str()));
		offset = atoi(strtok(NULL,delim.c_str()));

		if(m_pParam->pm_l_points!=num_points){
			cout<<"ZHPolar_Match: pm_points: "<<m_pParam->pm_l_points<<" read_points: "<<num_points<<endl;
			continue;
		}
		...
		for(int i=0;i<num_points;i++){
			...
			ls.bad[i] = 0;
			if(ls.r[i]<= m_pParam->pm_min_range){
...
bool CPolarMatch::readCarmon(string filename, string laser){
	...
	string delim(" ");
	string ourlaser("LMS"); // our log statrts with "LMS***"
	bool bIsOurlog(false);
	if(equal(ourlaser.begin(),ourlaser.end(),laser.begin())) // this is our log
	{
	...
	while(inf.getline(line,8192)){
		...
		if(bFirst){
			...
			pm_init();
			if(maxRange*100 != m_pParam->pm_max_range)
				m_pParam->pm_max_range = maxRange*100;
		}
		...
		// also read laser pose from carmon log
		if(!bIsOurlog) // this is ground_truth log
		{
			// remission data
	...
	cout<<"Scan size: "<<m_SickScans.size()<<endl;
	return (m_SickScans.size()>0);
}
...
// using PSM and ICP to calculate the error 
void CPolarMatch::testAccuracy(string filename)
{
	if(!readFlirt(filename)){
		cout<<"failed to read file: "<<filename<<endl;
		return ;
	...
	ofstream psm_log("/mnt/hgfs/SharedFold/log/err_psm.log");
	ofstream icp_log("/mnt/hgfs/SharedFold/log/err_icp.log");
	int cnt=0;
	bool bFirst = 1;	
	PMScan* ls;
	PMScan* ls_last;
	// ground truth is rendered to follow odometry format
	OrientedPoint2D last_pose;

	// number of failed match using psm and icp
	int n_failed_psm = 0;
	int n_failed_icp = 0;
	double dis_err,angle_err;
	
	// threshold for eliminating noisy data
	double odometry_thre = 0.6982; // 40 degree for th
	double dis_squre_thre = 1; // 1m 

	while(cnt<m_SickScans.size() ){
		ls = m_SickScans[cnt];
		cnt++;
		pm_preprocessScan(ls);
		if(bFirst){
			last_pose=OrientedPoint2D(ls->rx,ls->ry,ls->th);
			bFirst = false;
			ls_last = ls;
		}else{
			// get the trans from ground truth
			OrientedPoint2D gt_trans(last_pose.ominus(OrientedPoint2D(ls->rx,ls->ry,ls->th)));
			last_pose=OrientedPoint2D(ls->rx,ls->ry,ls->th);
...
#define SQURE(x) ((x)*(x))
			// calculate dis and angle error of psm
			OrientedPoint2D psm_trans(ls->rx/100.0,ls->ry/100.0,ls->th);
			dis_err = sqrt(SQURE(psm_trans.x-gt_trans.x)+SQURE(psm_trans.y-gt_trans.y));
			angle_err = fabs(psm_trans.theta - gt_trans.theta);
			if(dis_err < dis_squre_thre && angle_err < odometry_thre)
				psm_log<<dis_err<<"\t"<<angle_err<<endl;
			...
			try{
				...
				n_failed_icp++;
				cout<<"icp failed at "<<cnt<<endl; 
			}

			// calculate dis and angle error of icp
			OrientedPoint2D icp_trans(ls->rx/100.0,ls->ry/100.0,ls->th);
			dis_err = sqrt(SQURE(icp_trans.x-gt_trans.x)+SQURE(icp_trans.y-gt_trans.y));
			angle_err = fabs(icp_trans.theta - gt_trans.theta);
			if(dis_err < dis_squre_thre && angle_err < odometry_thre)
				icp_log<<dis_err<<"\t"<<angle_err<<endl;

			ls_last = ls;
		}//else
	}//while
	cout<<"Number of failed psm : "<<n_failed_psm<<endl;
	cout<<"Number of failed icp : "<<n_failed_icp<<endl;
	return ;
}
...
void CPolarMatch::runlog(int run_num) // run slam using log data
{
	...
	// ofstream trajectory("/mnt/hgfs/SharedFold/log/intel_psm.log");
 	ofstream trajectory("/mnt/hgfs/SharedFold/log/rawseed_psm.log");
	...
	// odometry threshold to forcast the accuracy of psm
	double odometry_thre = 0.6982; // 40 degree for th
	double dis_squre_thre = 10000; // 1m 
	float err_thre = 10; 
	...
	double orx,ory,oth; // to save the current laser pose
	PMScan* ls;
	PMScan* ls_last;
	
	int n_of_gt = 0 ; // number of pose using gt 
	int n_of_angle = 0; // number for big angle
	int n_of_err = 0; // number for big error 
	...
	while(cnt<m_SickScans.size() && cnt <run_num){
		...
		if(bFirst){
			...
			// cout<<"frame "<<cnt<<": "<<ls->rx<<" "<<ls->ry<<" "<<th<<endl;
		}else{

			// if odometry shows big change of angle, ==!
			if(fabs(ls->th-th)> odometry_thre){
				
				// calculate result from odometry
				OrientedPoint2D trans(last_pose.ominus(OrientedPoint2D(ls->rx,ls->ry,ls->th)));
				cur_pose = OrientedPoint2D(OrientedPoint2D((rx/100.0),(ry/100.0),th).oplus(trans));

				trajectory<<cur_pose.x<<" "<<cur_pose.y<<" "<<cur_pose.theta<<endl;
				...
				n_of_gt++;
				n_of_angle++;
				//cout<<"odo ("<<cnt-1<<","<<cnt-2<<") : "<<cur_pose<<endl;
				continue;
			}
			...
			try{
				pair<float,float> avg_err = pm_error_index2(ls_last,ls);
				pm_psm(ls_last,ls);
				cout<<"using PSM avr_err: x: "<<avg_err.first<<" y: "<<avg_err.second<<endl;
				pm_icp(ls_last,ls);
				avg_err = pm_error_index2(ls_last,ls);
				cout<<"using ICP avr_err: x: "<<avg_err.first<<" y: "<<avg_err.second<<endl;
			}catch(int err){
			...
			if(matchFailed){
				matchFailed = false;
				try{
					float avr_err = pm_icp(ls_last,ls);
					cout<<"using ICP avr_err: "<<avr_err<<endl;
				}catch(int err){
			...
			pair<float, float> err = pm_error_index2(ls_last,ls);
			float err_idx = sqrtf(err.first+err.second);
			// float err_idx = pm_error_index2(ls_last,ls);
			...
			double rel_dis = (xx-rx)*(xx-rx) + (yy-ry)*(yy-ry);
			/*bool err_f(false); bool dis_f(false);
			if(err_idx > err_thre ) err_f = true;
			if(rel_dis > dis_squre_thre) dis_f = true;*/

			if( err_idx> err_thre || rel_dis > dis_squre_thre || matchFailed){
				
				// calculate result from odometry
				OrientedPoint2D trans(last_pose.ominus(OrientedPoint2D(orx,ory,oth)));
				cur_pose = OrientedPoint2D(OrientedPoint2D((rx/100.0),(ry/100.0),th).oplus(trans));

				ls_last = ls;
				last_pose = OrientedPoint2D(orx,ory,oth);
				/*rx = orx*100;
				ry = ory*100;
				th = oth;*/
				rx = cur_pose.x*100;
				...
				trajectory<<(rx/100.0)<<" "<<(ry/100.0)<<" "<<th<<endl;
				n_of_gt++;
				/*if(err_f) n_of_err++;
				if(dis_f) n_of_dis++;*/
				n_of_err++;
				// cout<<"err: ("<<cnt<<","<<cnt-1<<") : "<<cur_pose<<endl;
				continue;
			}
		...
		// cout<<"matched ("<<cnt<<","<<cnt-1<<") : "<<(ls->rx/100.0)<<" "<<(ls->ry/100.0)<<" "<<ls->th<<endl;
		// cout<<"matched "<<cnt<<": "<<(xx/100.0)<<" "<<(yy/100.0)<<" "<<tth<<endl;
	...
	trajectory.close();
	cout<<"Number of GT: "<<n_of_gt<<endl;
	cout<<"Number for big angle: "<<n_of_angle<<endl;
	cout<<"Number for big err: "<<n_of_err<<endl;
	return ;
}

bool CPolarMatch::construct511Data(char * line, PMScan& scan)
{
	string delim(" ");
	string timestamp = strtok(line,delim.c_str());
	unsigned int num_points = atoi(strtok(NULL,delim.c_str()));
	if(num_points != m_pParam->pm_l_points){
		cout<<"failed to read sick 511!"<<endl;
		return false;
	}
	for(int i=0;i<num_points;i++){
		scan.r[i] = atof(strtok(NULL,delim.c_str()))*100.;
		scan.x[i] = scan.r[i]*pm_co[i];
		scan.y[i] = scan.r[i]*pm_si[i];
		scan.bad[i] = 0;
		if(scan.r[i] < m_pParam->pm_min_range)
			scan.r[i] = m_pParam->pm_max_range + 1;
	}
	return true;
}

namespace{
	ostream& operator<<(ostream& out,const PMScan& scan){
		for(int i=0;i<scan.np;i++){
			out<<scan.r[i]<<" "<<scan.x[i]<<" "<<scan.y[i]<<" "<<scan.bad[i]<<" "<<scan.seg[i]<<endl;
		}
		return out;
	}
}

void CPolarMatch::run511Data(string filename, int run_num) // run slam using log data
{
 	ofstream trajectory("/mnt/hgfs/SharedFold/log/psm/psm511my.log");
	bool bFirst = 1;
	int cnt = 0;
	if(run_num < 0)
		run_num = 1000001;
	...
	double rx=0,ry=0,th=0;//robot starts at 0,0,0
	PMScan ls(m_pParam->pm_l_points);
	PMScan ls_last(m_pParam->pm_l_points);
	...
	char line[4096];
	ofstream input("input_psm.log");
	while(inf.getline(line,4096) && cnt <run_num){
		if(!construct511Data(line, ls))
			continue;
		cnt++;
		// input<<ls<<endl;
		pm_preprocessScan(&ls);
...
void CPolarMatch::runlogImproved(int run_num)	// rum slam using log data
{
	...
	// trajectory record
#ifdef NEW_OP
	ofstream trajectory("/mnt/hgfs/SharedFold/zhnew.log");
#endif
#ifdef VEC_OP
	ofstream trajectory("/mnt/hgfs/SharedFold/zhvec.log");
#endif
	if(!trajectory.is_open())
	{
		cout<<"failed to open output trajectory!"<<endl;
	...
	while(cnt<m_SickScans.size() && cnt <run_num){
		...
		if(bFirst){
			...
			pair<PM_TYPE,PM_TYPE> err = pm_error_index2(ls_last,ls);
			PM_TYPE err_idx = sqrtf(err.first+err.second);
			// PM_TYPE err_idx = pm_error_index2(ls_last,ls);
			bool refSwitched = false;
			if(matchFailed || err_idx > 0.05) // 5 cm 
			{
...
PM_TYPE CPolarMatch::pm_translation_estimation(const PMScan *ref, const PM_TYPE *new_r, const int *new_bad, PM_TYPE C, PM_TYPE *dx, PM_TYPE *dy)
{
	...
	vector<float> err_vec(m_pParam->pm_l_points);
	vector<float> err_vec_x(m_pParam->pm_l_points); // record variance along x-axis
	vector<float> err_vec_y(m_pParam->pm_l_points); // record variance along y-axis
	for ( i=0;i<m_pParam->pm_l_points;i++ )
	{
		...
		{
			/*float dis = fabs(dr)*0.01;
			float dis_x,dis_y;
			// record points whose dis < 1m
			if(dis < 1){
				err_vec[n_valid_points] = dis;
				dis_x = dis*pm_co[i];
				dis_y = dis*pm_si[i];
				err_vec_x[n_valid_points] = SQ(dis_x);
				err_vec_y[n_valid_points] = SQ(dis_y);
				n_valid_points++;
			}*/
	...
	*dy = inv21*hw1+inv22*hw2;
	...
	// calculate abs_err using MIN_ERR_POINTS matched points
	// sort(err_vec.begin(),err_vec.begin()+n_valid_points);
	// int N = (int)((double)n_valid_points*0.2);
	// int N = n_valid_points*0.5 > MIN_ERR_POINTS ? n_valid_points*0.5 : MIN_ERR_POINTS;
	/*float abs_err_x = 0; 
	float abs_err_y = 0;
	for(int i=0; i< n_valid_points; i++)
	{
		abs_err_x += err_vec_x[i];
		abs_err_y += err_vec_y[i];
	}
	return make_pair(abs_err_x/n_valid_points,abs_err_y/n_valid_points);
*/
	// return (abs_err/N);
	return(abs_err/n);
...
*/
pair<PM_TYPE,PM_TYPE> CPolarMatch::pm_error_index2 ( PMScan *ref,PMScan *cur, int* associatedPoints )
{
	...
	vector<float> err_x(m_pParam->pm_l_points);
	vector<float> err_y(m_pParam->pm_l_points);
	float sum_err_x = 0;
	float sum_err_y = 0;

	for ( int i=0;i < m_pParam->pm_l_points;i++ ) //searching through the current points
	{
		PM_TYPE delta = fabsf ( new_r[i] - ref->r[i] )*0.01; // from cm to m
		if ( !new_bad[i] && !ref->bad[i] && delta < PM_MAX_ERROR / 2.0)
		{
			// matched points less than 1m
			if(delta < 1){
				e += delta;
				sum_err_x += SQ(delta*pm_co[i]);
				sum_err_y += SQ(delta*pm_si[i]);
				n++;
			}
		}
	}//for i

	float avg_err_x, avg_err_y;
	if ( n > 0 )
	{
		avg_err = e/n;
	...
	// return avg_err;
	return make_pair(avg_err_x,avg_err_y);
}
...
#define INTERPOLATE_ICP  //comment out if no interpolation of ref. scan points iS  necessary
	...
	// for record relative err_x and err_y
	vector<float> this_x(m_pParam->pm_l_points);
	vector<float> that_x(m_pParam->pm_l_points);
	vector<float> this_y(m_pParam->pm_l_points);
	vector<float> that_y(m_pParam->pm_l_points);
	int n_valid_points;
...
#endif
					// record correspondings
					this_x[i] = nx[index[i][0]]; this_y[i] = ny[index[i][0]];
					that_x[i] = ix[index[i][1]]; that_y[i] = iy[index[i][1]];
	...
	for(int i=0;i<n_valid_points;i++)
	{
		abs_err += sqrtf(SQ(this_x[i]-that_x[i]) + SQ(this_y[i]-that_y[i]));
	}
	// return ( abs_err/n );
	return (abs_err/n_valid_points);
}//pm_icp
...
bool CPolarMatch::pm_is_corridor( PMScan *act )
{
	PM_TYPE fi1=0,fi2=0,fi3=0;
	PM_TYPE sxx=0,sx=0,std1,std2;
	PM_TYPE n=0;

	for ( int i=0;i< ( /*PM_L_POINTS*/m_pParam->pm_l_points -1 );i++ )
	{
