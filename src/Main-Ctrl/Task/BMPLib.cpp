...
pthread_mutex_t g_Critical_BMPLibRun;
BMPLib::BMPLib(void)
{
	pthread_mutex_init(&g_Critical_BMPLibRun,0);
}
...
{
	//pthread_mutex_lock(&g_Critical_BMPLibRun);
	...
	if(pfile!=NULL)
	{
		printf("file %s.bmp open success.\n",pFile);
		//读取位图文件头信息
		WORD fileType;
	...
	printf("BMP Bug333333\n");
	//将位图数据转化为RGB数据
	//libRGBQUAD* dataOfBmp=new libRGBQUAD[width*height*sizeof(libRGBQUAD)] ;
	//dataOfBmp = (libRGBQUAD *)malloc(width*height*sizeof(libRGBQUAD));//用于保存各像素对应的RGB数据
	
//printf("sizeof(libRGBQUAD) == %d  \n",sizeof(libRGBQUAD));
	//memset(dataOfBmp,0,width*height*4);
	//printf("BMP Bug44444\n");
/*	if(bitInfoHead.biBitCount<24)//有调色板，即位图为非真彩色
	...
	*pnWidth=width;
	...
	//delete [] dataOfBmp;
	delete [] pColorData;
	//pthread_mutex_unlock(&g_Critical_BMPLibRun);
	printf("file %s.bmp wrtite success.\n",pFile);
	return pucData;
}
