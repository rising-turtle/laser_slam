...
void
write_laser_data2( FILE *fout, unsigned long sec, unsigned long usec,
		  int numLaserv1, int *laserval1 )
{
  int i;
  int offset = 0;
  fprintf( fout, "%ld.%ld, %d, %d,", sec, usec, numLaserv1, offset );
  for (i=0; i<numLaserv1; i++) {
  	if(i==numLaserv1-1){
	     fprintf( fout, " %f", (float)(laserval1[i]/100.) );
	}
	else fprintf( fout, " %f,", (float)(laserval1[i]/100.) );
  }
  fprintf( fout, "\n" );
}
...
main( int argc, char *argv[] )
{
  ...
  if((fpose = fopen("intel_pose.log","w+")) == NULL){
  	printf("failed to open file pose.log\n");
	return -1;
  }
  ...
  if (argc==3) {
	...
	int offset = 0;
	  fprintf( fpose, "%ld.%ld, %d, %d, %d, %f, %f, %f\n", sec, usec, offset, offset, offset, (float) (cur_posx/100.), (float) (cur_posy/100.), (float) D2R(cur_poso) );
	  ...
	  if (1 || lasernr==0) {
	    ...
	    /*write_laser_data( fpout,
			      numValues1, laser1,
			      numValues2, laser2 );*/
	      write_laser_data2(fpout, sec, usec, numValues1, laser1);
	    numValues2 = 0;
	  } else {
	    for (i=0; i<numValues; i++) {
